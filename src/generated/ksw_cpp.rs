        ...
        unsafe {
            ...
            'outer: for i in 0..tlen_us {
                ...
                while j < slen_us {
                    ...
                    h = _mm_subs_epu8(h, shift_v);
                    let mut e = *e_buf.get_unchecked(j);
                    h = _mm_max_epu8(h, e);
                    h = _mm_max_epu8(h, f);
                    max_v = _mm_max_epu8(max_v, h);
                    *h1.get_unchecked_mut(j) = h;
                    // E'(i+1, j) = max(E - e_del, H - oe_del)
                    e = _mm_subs_epu8(e, e_del_v);
                    let t = _mm_subs_epu8(h, oe_del_v);
                    e = _mm_max_epu8(e, t);
                    *e_buf.get_unchecked_mut(j) = e;
                    // F'(i, j+1) = max(F - e_ins, H - oe_ins)
                    f = _mm_subs_epu8(f, e_ins_v);
                    let t = _mm_subs_epu8(h, oe_ins_v);
                    f = _mm_max_epu8(f, t);
                    // Prepare h = H0[j] for the next stripe iteration.
                    h = *h0.get_unchecked(j);
                    j += 1;
                }
                // Lazy F propagation (SWPS3-style). 16 max passes are sufficient for u8.
                'lazyf: for _k in 0..16 {
